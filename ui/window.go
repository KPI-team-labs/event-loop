...
func (pw *Visualizer) Main() {
	...
	pw.pos.Max.X = 200
	pw.pos.Max.Y = 200
	pw.tetCenter.X = WindowWidth / 2
	pw.tetCenter.Y = WindowHeight / 2
	driver.Main(pw.run)
}
...
func (pw *Visualizer) handleEvent(e any, t screen.Texture) {
	...
	case mouse.Event:
		if t == nil {
			// Реакція на натискання правої кнопки миші.
			if e.Button == mouse.ButtonRight && e.Direction == mouse.DirPress {
				pw.tetCenter = image.Point{
					int(e.X),
					int(e.Y),
				}
...
func (pw *Visualizer) drawDefaultUI() {
	...
	// Відображення фігури Т синього кольору.
	x, y := pw.tetCenter.X, pw.tetCenter.Y
